...
export default function ReceivedItemsReportsPage() {
  const { receivedItems } = useInventory()
  ...
  const exportToPDF = async () => {
    ...
    try {
      ...
      doc.setFontSize(16)
      doc.text("Received Items Report", pageWidth / 2, 25, { align: "center" })
      doc.setFontSize(10)
      ...
      link.href = url
      link.download = `hesu-received-items-report-${new Date().toISOString().split("T")[0]}.pdf`
  ...
  return (
    <SidebarInset className="h-full bg-white">
      <header className="flex h-16 shrink-0 items-center gap-2 border-b px-4 bg-white">
        ...
        <h1 className="text-lg font-semibold">Received Items Report</h1>
      </header>
      ...
      <div className="flex flex-1 flex-col gap-6 p-4 sm:p-6 w-full bg-white min-h-[calc(100vh-4rem)]">
        <Card className="border-none shadow-md bg-white w-full">
          <CardHeader>
            <CardTitle>Received Items Report</CardTitle>
            <CardDescription>View and export received items information</CardDescription>
          </CardHeader>
            ...
            <div className="space-y-6">
              <div className="flex flex-col gap-4 md:flex-row md:items-center">
                <div className="flex-1">
                  <Input
                    placeholder="Search item type, toner model, supplier, or printer..."
                    value={searchTerm}
              ...
              <div className="flex justify-between items-center mb-4">
                ...
                <div className="text-sm text-gray-600">
                  Showing {paginatedItems.length > 0 ? (currentPage - 1) * itemsPerPage + 1 : 0} - {Math.min(currentPage * itemsPerPage, filteredItems.length)} of {filteredItems.length} received items
                </div>
              </div>
              ...
              <div className="rounded-md border shadow-sm overflow-hidden bg-white">
                <Table className="font-inter w-full">
                  <TableHeader>
                    <TableRow className="bg-gradient-to-r from-green-50 to-green-100 hover:bg-gradient-to-r hover:from-green-50 hover:to-green-100">
                  ...
                  <TableBody>
                    {paginatedItems.length === 0 ? (
                      ...
                      paginatedItems.map((item, idx) => (
                        <TableRow key={item.id} className={`transition hover:bg-green-50 ${idx % 2 === 0 ? 'bg-white' : 'bg-gray-50/50'}`}>
                      ...
                      if (start > 1) {
                        pages.push(
                          <button
                            ...
                            className="px-3 py-1 rounded border text-sm font-medium bg-white text-green-600 border-green-200 hover:bg-green-50"
                          >
                            1
                          </button>
                        )
