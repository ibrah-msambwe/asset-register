...
import { ArrowLeft, PackageCheck, Save, Loader2, Users, Search, Plus, MoreHorizontal, Edit, Trash2, Eye } from "lucide-react"
...
export default function IssueItemsPage() {
  ...
  useEffect(() => {
    if (formData.itemType) {
      const users = getEligibleUsersForItem(formData.itemType as ItemType)
      setEligibleUsers(users)
      if (users.length === 0) {
        setFormError(`No eligible users found for ${formData.itemType}. Users must have related assets assigned.`)
      } else {
        setFormError(null)
      }
    } else {
  ...
  }, [formData.itemType, getEligibleUsersForItem])
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.issuedTo) {
      setFormError("Issued to is required.")
      setLoading(false)
      return
    }
    ...
    if (eligibleUsers.length > 0 && !eligibleUsers.includes(formData.issuedTo)) {
      setFormError("Selected user is not eligible for this item type based on their assigned assets.")
      setLoading(false)
      return
    }

    try {
      const editingId = (window as any).editingIssuedItemId
      let assetId: string | undefined
      const selectedPrinter = formData.printerId ? printers.find((p) => p.id === formData.printerId) : undefined
      const printerDevice = selectedPrinter ? devices.find((d) => d.id === selectedPrinter.id) : undefined
      
      if (formData.itemType === "Keyboard" || formData.itemType === "Mouse") {
        const userDevice = devices.find(
          (d) =>
            (d.type === "Computer" || d.type === "Laptop") &&
            d.assignedTo === formData.issuedTo
        )
        assetId = userDevice?.id
      } else if (formData.itemType === "Toner") {
        assetId = formData.printerId
      } else if (formData.itemType === "Monitor") {
        const userDevice = devices.find(
          (d) => d.type === "Computer" && d.assignedTo === formData.issuedTo
        )
        assetId = userDevice?.id
      }
      ...
      if (editingId) {
        await updateIssuedItem(editingId, itemData)
        toast.success("Item updated successfully! ✏️", {
          description: `${formData.quantity} ${formData.itemType}(s) have been updated.`,
          duration: 4000,
        })
        delete (window as any).editingIssuedItemId
      } else {
        await addIssuedItem(itemData)
        toast.success("Items issued successfully! 📤", {
          description: `${formData.quantity} ${formData.itemType}(s) issued to ${formData.issuedTo}.`,
          duration: 4000,
        })
      }

      if (!(window as any).editingIssuedItemId) {
        setFormData({
          itemType: "" as ItemType | "",
          tonerModel: "" as TonerModel | "",
          tonerType: "" as TonerType | "",
      ...
      setFormError("Error issuing items.")
  ...
  const handleChange = (field: string, value: string) => {
    setFormData((prev) => {
      ...
      if (field === "printerId") {
        ...
        if (selectedPrinter && selectedPrinter.assignedTo) {
          updated.issuedTo = selectedPrinter.assignedTo
        }
      }
      return updated
    })
  ...
  return (
    <SidebarInset className="h-full bg-white">
      <header className="flex h-16 shrink-0 items-center gap-2 border-b px-4 bg-white">
        ...
        <div>
          <h1 className="text-lg font-semibold">Issue Items</h1>
          <p className="text-xs text-muted-foreground">Issue inventory items to users</p>
        </div>
      </header>
      <div className="flex flex-1 flex-col gap-6 p-6 bg-white">
          ...
          <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
            ...
            <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
              <DialogHeader>
                ...
                <DialogDescription>
                  Issue inventory items to employees based on their assigned assets
                </DialogDescription>
              </DialogHeader>
              ...
              {eligibleUsers.length > 0 && (
                <Alert className="col-span-full">
                  ...
                  <AlertDescription>
                    <strong>Smart Integration:</strong> Showing {eligibleUsers.length} eligible user(s) based on their assigned assets.
                  </AlertDescription>
                </Alert>
              )}
              ...
              <div className="flex flex-col gap-1 w-full">
                <Label htmlFor="issuedTo" className="flex items-center gap-2">
                ...
                {eligibleUsers.length > 0 && (
                  <p className="text-xs text-muted-foreground mt-1">
                    System-suggested users based on asset assignments
                  </p>
                )}
              </div>
        ...
        <Card className="border-none shadow-xl bg-gradient-to-b from-white to-blue-50 rounded-2xl">
          <CardHeader className="pb-4">
            <div className="flex items-center justify-between">
              <div>
                <CardTitle className="text-xl font-bold">Issued Items History</CardTitle>
                <CardDescription>View all issued items and their details</CardDescription>
              </div>
          ...
          <CardContent>
            {filteredIssuedItems.length === 0 ? (
              <div className="text-center py-12 text-gray-500">
                ...
                <p className="text-lg font-medium">No issued items found</p>
                <p className="text-sm mt-2">
                  {searchTerm ? "Try adjusting your search terms" : "Start issuing items to see them here"}
                </p>
              </div>
            ) : (
                ...
                <Table>
                  <TableHeader>
                    <TableRow className="bg-blue-50 hover:bg-blue-50">
                      ...
                      <TableHead className="border-r border-dashed border-gray-200 font-semibold">Issued To</TableHead>
                      <TableHead className="border-r border-dashed border-gray-200 font-semibold">Printer</TableHead>
                      <TableHead className="border-r border-dashed border-gray-200 font-semibold">Issue Date</TableHead>
                  ...
                  <TableBody>
                    {filteredIssuedItems.map((item) => (
                      <TableRow key={item.id} className="hover:bg-blue-50/50">
                        ...
                        <TableCell className="border-r border-dashed border-gray-200">
                          <span className="text-sm">{formatDate(item.issuedDate)}</span>
                        </TableCell>
                        <TableCell>
                            ...
                            <DropdownMenuContent align="end">
                              <DropdownMenuItem onClick={() => {
                                const details = [
                                  ...
                                  item.tonerModel && `Toner Model: ${item.tonerModel}`,
                                  item.tonerType && `Toner Type: ${item.tonerType}`,
                                  item.tonerColor && `Toner Color: ${item.tonerColor}`,
                                  `Quantity: ${item.quantity}`,
                                  `Issued To: ${item.issuedTo}`,
                                  item.printerName && `Printer: ${getPrinterDisplay(item.printerId, item.printerName)}`,
                                  `Issue Date: ${formatDate(item.issuedDate)}`,
                              ...
                              <DropdownMenuItem onClick={() => {
                                ...
                                setDialogOpen(true)
                                ;(window as any).editingIssuedItemId = item.id
                              }}>
                              ...
                              <DropdownMenuItem 
                                onClick={async () => {
                                  if (confirm(`Are you sure you want to delete this ${item.itemType} issue?`)) {
                                    try {
                                      await deleteIssuedItem(item.id)
                                      toast.success("Issue deleted successfully! 🗑️", {
                                        description: `${item.itemType} issue has been removed.`,
                                        duration: 3000,
                                      })
                                    } catch (error) {
                                      toast.error("Error", {
